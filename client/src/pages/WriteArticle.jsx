        ...
        <div className='flex items-center gap-3'>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {articleLength.map((item, index)=>(
              <span onClick={()=> setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text===item.text ? 'bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-500'}`} key={index}>{item.text}</span>
            ))}
          </div>
          <br />
